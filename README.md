...
> Trying it without installing? `npx mappamind status <repo>` works for a quick look, but the lifecycle hooks invoke the bare `mappamind` binary, so hooks need the global install above.
...
Install globally with `npm i -g mappamind`, then reinstall hooks so they capture a stable binary path:
